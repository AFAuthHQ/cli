...
func TestReplaceCreatesBackupAndInstallsNew(t *testing.T) {
	...
	returnedBackup, err := id2.Replace(path)
	if err != nil {
		t.Fatalf("Replace: %v", err)
	}
	if returnedBackup == "" {
		t.Fatalf("Replace did not return a backup path for an existing key")
	}
	...
	if backupPath != returnedBackup {
		t.Fatalf("returned backup %q does not match discovered backup %q", returnedBackup, backupPath)
	}
...
func TestReplaceOnMissingFileJustWritesNew(t *testing.T) {
	...
	backup, err := id.Replace(path)
	if err != nil {
		t.Fatalf("Replace on missing: %v", err)
	}
	if backup != "" {
		t.Fatalf("Replace on a missing file should return no backup, got %q", backup)
	}
